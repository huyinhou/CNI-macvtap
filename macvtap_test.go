...
var _ = Describe("macvtap Operations", func() {
	...
	It("deconfigures an unconfigured macvtap link with DEL", func() {
		const IFNAME = "macvl0"
	...
	It("configures and deconfigures a l2 macvtap link with ADD/DEL", func() {
		const IFNAME = "macvl0"
	...
	It("configures and deconfigures a cniVersion 0.4.0 macvtap link with ADD/DEL", func() {
		const IFNAME = "macvl0"
    ...
    "ipam": {
		...
		cniVersion := "0.4.0"
		newConf, err := buildOneConfig("macvtapTestv4", cniVersion, n, result)
		Expect(err).NotTo(HaveOccurred())
		...
		// CNI Check on macvtap in the target namespace
		err = originalNS.Do(func(ns.NetNS) error {
			defer GinkgoRecover()
	...
	It("configures and deconfigures a macvtap link with ADD/DEL, without master config", func() {
		const IFNAME = "macvl0"
